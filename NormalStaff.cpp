/**
 * @file NormalStaff.cpp
 * @brief Implements the NormalStaff request handler.
 */

#include "NormalStaff.h"

void NormalStaff::handleRequest(const string& request) {
    if (request.compare("care request") == 0 ||
        request.compare("delivery") == 0 ||
        request.compare("customer") == 0) {
